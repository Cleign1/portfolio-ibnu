...
export function ExperiencePreviewClient({ initialData, serverURL }: Props) {
  const { data } = useLivePreview<ExperienceType>({
    initialData,
    serverURL,
    depth: 1,
  })

  const title = data.title || "Where I've worked"
  const blocks = data.experienceBlocks || []
      ...
      <div className="mx-auto max-w-[760px] px-6 md:px-20 lg:px-20">
        ...
        <div className="space-y-0">
          {blocks.map((block, index) => {
            if (block.blockType === 'experienceItem') {
              return (
